/*
 * @Author: guk 
 * @Date: 2019-07-10 10:50:22 
 * @Last Modified by: guk
 * @Last Modified time: 2019-07-10 14:26:20
 * 工具栏图标
 */
  ...
  >
    ...
    >
      ...
      >
        ...
        <div class="preview-title">{{ info.appTitle || info.config.app.title }}</div>
    ...
    <!-- 图标 -->
    <div class="task-bar-icon-main" :class="{ 'app-open': info.config.window.status === 'open' }" :data-name="info.appName || info.config.app.name">
      <img class="app-icon" :class="{ 'app-icon-down': isMouseDown}" v-if="info.config.app.icon" :src="info.config.app.icon" :data-name="info.appName || info.config.app.name">
    ...
    methods: {
      ...
      onIconRightClick: function (event) {
        ...
        _t.$nextTick(function () {
          ...
          let appName = event.target.dataset['name'] || _t.info.appName || _t.info.config.app.name || null
          let appInfo = {..._t.info}
          // 菜单信息
          let contextMenuInfo = {
            isShow: true,
      ...
      onIconMouseOver: function () {
        ...
        if (appInfo.config.window.type === 'iframe') {
          ...
          let appName = appInfo.appName || appInfo.config.app.name
